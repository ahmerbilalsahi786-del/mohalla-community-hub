      ...
      <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
        <SheetTrigger asChild>
          <button className="flex h-16 min-w-0 flex-1 flex-col items-center justify-center gap-0.5">
            <div className={cn(
              'flex h-7 w-7 items-center justify-center rounded-lg transition-colors sm:h-8 sm:w-8',
              moreLinks.some((item) => location === item.href || (item.href !== '/' && location.startsWith(item.href))) ? 'bg-primary text-primary-foreground shadow-sm' : ''
            )}>
              <Menu size={18} className={moreLinks.some((item) => location === item.href || (item.href !== '/' && location.startsWith(item.href))) ? 'text-primary-foreground' : 'text-muted-foreground'} />
            </div>
            <span className={cn('truncate text-[9px] font-bold sm:text-[10px]', moreLinks.some((item) => location === item.href || (item.href !== '/' && location.startsWith(item.href))) ? 'text-primary' : 'text-muted-foreground')}>
              More
            </span>
          </button>
        </SheetTrigger>
