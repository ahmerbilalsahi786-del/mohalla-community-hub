...
create or replace function public.repair_member_invite_profiles()
returns integer
language plpgsql
security definer
set search_path = ''
...
begin
  with invited_members as (
    select
      u.id as user_id,
      (u.raw_user_meta_data->>'join_community_id')::uuid as invited_community_id
    from auth.users u
    where u.raw_user_meta_data->>'registration_type' = 'member'
      and nullif(trim(coalesce(u.raw_user_meta_data->>'join_community_id', '')), '') is not null
      and (u.raw_user_meta_data->>'join_community_id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  )
  ...
  set
    ...
    is_verified = false,
    updated_at = now()
  from invited_members i
  join public.community_settings c
    on c.id = i.invited_community_id
...
revoke execute on function public.repair_member_invite_profiles() from public, anon, authenticated;

select public.repair_member_invite_profiles();
